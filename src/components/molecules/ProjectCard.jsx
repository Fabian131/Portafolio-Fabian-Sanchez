  ...
  return (
    <GlassCard tilt={true} className="p-6 flex flex-col gap-6 cursor-pointer h-full border-t border-t-cyan-500/30">
      <div className="w-full h-40 sm:h-52 md:h-64 rounded-2xl bg-gradient-to-br from-cyan-500/20 to-blue-500/20 flex items-center justify-center overflow-hidden relative group-hover:shadow-[0_0_30px_rgba(14,165,233,0.3)] transition-shadow duration-500">
         ...
         <h3 className="text-2xl sm:text-3xl font-black z-10 drop-shadow-lg text-gray-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-300 group-hover:-translate-y-2 transition-all duration-300">{title}</h3>
      </div>
      <div className="flex-1 flex flex-col">
        <h3 className="text-2xl font-bold mb-4 flex justify-between items-center text-gray-900 dark:text-white">
          {title}
          <a href={link || '#'} target="_blank" rel="noopener noreferrer" className="text-gray-500 hover:text-cyan-500 transition-colors"><ExternalLink size={20} /></a>
        </h3>
        ...
        <div className="flex flex-wrap gap-2 text-xs font-medium mt-auto">
          {tags.map((tag, index) => (
            <span key={index} className="px-3 py-1.5 rounded-full bg-cyan-500/10 text-cyan-700 dark:text-cyan-300 border border-cyan-500/20">{typeof tag === 'string' ? tag : tag.label}</span>
          ))}
        </div>
      </div>
    </GlassCard>
